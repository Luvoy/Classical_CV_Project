...
def findScaleSpaceExtrema(gaussian_images,
    ...
    threshold = floor(0.5 * contrast_threshold / num_intervals *
    ...
    for i in range(-radius, radius + 1):
        ...
        if region_y > 0 and region_y < image_shape[0] - 1:
            for j in range(-radius, radius + 1):
                region_x = int(round(
                    keypoint.pt[0] / float32(2**octave_index))) + j
...
def generateDescriptors(keypoints,
    ...
    for keypoint in keypoints:
        ...
        half_width = int(
            round(hist_width * sqrt(2) * (window_width + 1) *
                  0.5))  # sqrt(2) corresponds to diagonal length of a pixel
        half_width = int(
            min(half_width,
                sqrt(num_rows**2 +
                     num_cols**2)))  # ensure half_width lies within image
        ...
        descriptor_vector = histogram_tensor[
            1:-1, 1:-1, :].flatten()  # Remove histogram borders
